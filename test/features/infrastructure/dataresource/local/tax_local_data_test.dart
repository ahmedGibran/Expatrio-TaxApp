...
void main() {
  ...
  group('taxLocalDataImpl test group', () {
    ...
    test("should call cacheTaxData Method and store the tax obj as string",
        () async {
      await taxLocalDataImpl.cacheTaxData(tax);
      verify(mockStorage.write(
          key: CACHED_TAX_DATA, value: jsonEncode(tax.toJson())));
    });

    test("should call getTaxData Method and return the stored tax obj",
        () async {
      when(mockStorage.read(key: CACHED_TAX_DATA))
          .thenAnswer((_) async => jsonEncode(tax.toJson()));
